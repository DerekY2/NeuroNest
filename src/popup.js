...
Interface.nodeSelectors.forEach(selector => {
  selector.addEventListener("click", (e) => {
    ...
    } else if (selector.classList.contains('inject-active')) {
      // Inject the script into the currently active tab
      chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        if (tabs[0]) {
          injectScript(tabs[0].id, selector.dataset.injection);
        }
      });
    } else if (selector.dataset.url) {
  ...
  if (checkOpen) {
    chrome.tabs.query({ currentWindow: true, url: request_url }, tabs => {
      if (tabs && tabs.length > 0) {
        ...
        if (file) injectScript(targetTab, file);
      } else {
        chrome.tabs.create({ url: request_url }, tab => {
          if (file) injectScript(tab.id, file);
        });
      }
    });
  }
